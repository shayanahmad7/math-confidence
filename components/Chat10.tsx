...
// Define props so we can receive userId, chapter, section.
...
const Chat2: React.FC<Chat2Props> = ({ userId, chapter, section }) => {
  // Use the assistant hook, passing userId, chapter, and section in the body.
  const { status, messages: aiMessages, input, submitMessage, handleInputChange, stop } = useAssistant({
    api: "/api/assistant2",
    body: {
      userId,                 // The user's ID (passed from parent)
      chapter,                // e.g. "Chapter 1: Whole Numbers"
      section,                // e.g. "Rounding Numbers"
      assistantId: process.env.ASSISTANT2_ID,
    },
  });
  ...
  useEffect(() => {
    ...
    const initialMessages: Message[] = [
      {
        id: "initial-1",
        content: `Welcome to the section "${section}" in "${chapter}". We'll go through this step by step. Feel free to ask any questions along the way!`,
        role: "assistant",
      },
      {
        id: "initial-2",
          ...
          >
            ...
            >
              ...
              <div
                ...
                } prose-headings:text-inherit prose-p:text-inherit prose-strong:text-inherit prose-ol:text-inherit prose-ul:text-inherit [&_.katex-display]:my-3 [&_.katex-display]:text-center`}
              >
